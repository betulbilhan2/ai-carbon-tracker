...
CarbonEmission — anket verisinden "profil özelliklerine dayalı beklenen karbon ayak izi" tahmini (dikkat: "kişisel baseline" değil — model kullanıcıyı geçmişte gözlemlemiyor, sadece benzer profildeki gözlemlerden genelleme yapıyor)





Anlık/günlük hesaplama



ML değil, deterministik formül (lookup tablolar)
...
TabNet'ten bağımsız, paralel katman
...
Havuzdan seçim (V1: kural tabanlı, V2: bandit)
...
Sadece seçilen öneriyi doğal dile çevirme — öneri seçmez, karbon hesabı yapmaz, cluster belirlemez
...
Kullanılmıyor. Yerine deviation_score (beklenenden yüzdesel sapma) — istatistiksel anomali iddiası yapmıyoruz, sadece "müdahale eşiği" tanımlıyoruz
...
Kapsam dışı; rapora yumuşatılmış V2 notu olarak yazılacak
...
Diet, Transport, Vehicle Type, Heating Energy Source, Cooking_With, Social Activity
...
Energy efficiency, Waste Bag Size, Frequency of Traveling by Air, How Often Shower



Sıralı encode (0/1/2/3)
...
StandardScaler (K-Means mesafe tabanlı; ölçeklenmezse büyük sayılı kolon kümelemeye domine eder)





Çok bileşenli (bitmask)
...
K-Means cluster'ları ve deviation_score, öneri SEÇİMİNDE artık zorunlu girdi değil.
...
**"Etki büyüklüğü" artık min_deviation_score ile değil, TabNet'in what-if farkıyla

hesaplanıyor:** sinir_deger koşulunu sağlayan her kural için, hedef_sutun kullanıcının
 satırında simulasyon_hedefi ile değiştirilip TabNet'e tekrar sorulacak; mevcut_tahmin -  simulasyon_tahmini farkı, o önerinin somut "X kg CO2 azaltır" etkisini verecek.



Sürekli sütunlarda birden fazla eşik aynı anda tetiklenebilir (ör. 3000 km/ay kullanan
...
LLM'e hâlâ sadece seçilmiş kuralın id'si (+ mesajı) verilecek; LLM öneri aramayacak/
...
carbon_emission.csv + carbon_emission (1).csv



TabNet eğitim verisi
...
Hesaplama motoru — beslenme lookup
...
Hesaplama motoru — ulaşım lookup
...
Dashboard karşılaştırma (ML'e girmiyor)
...
V2'de olası segmentasyon referansı (ML'e girmiyor)
...
ML'e girmiyor, literatür desteği olarak kullanılabilir
...
Öneri havuzu zenginleştirme (ML'e girmiyor)
...
Şema/eksik veri kontrolü, carbon_emission ikilisinin birleştirilmesi, lookup tabloları oluşturma



tabnet_*.parquet, lookup_tables/*
...
02_tabnet_feature_engineering.ipynb
...
Encode/ölçekleme (sadece train'e fit), rastgele train/val/test



feature_metadata.json, işlenmiş veri
...
03_tabnet_egitim_ve_baseline.ipynb
...
Mean/Median → Random Forest → TabNet, MAE/RMSE/R² karşılaştırması, feature importance (ayrı analiz)



tabnet_model_v1.zip, karşılaştırmalı metrikler
...
Deterministik actual_weekly_kg hesaplayıcı, birim testleri
...
05_kullanici_kumeleme.ipynb



Notebook 1 çıktısı (Bölüm 2'deki feature seti)



K-Means, elbow/silhouette, küme profillerinin insan yorumuyla etiketlenmesi



kmeans_model_v1.pkl, cluster_profiles_v1.csv
...
06_oneri_secimi_kural_tabanli.ipynb



Öneri havuzu (bekleniyor), cluster + deviation_score



Kural tabanlı seçim mantığı + functional test seti (ör. "transport deviation yüksekse transport önerisi gelmeli mi" kontrolü)
...
07_llm_mesajlastirma_ve_entegrasyon.ipynb
...
LLM mesajlaştırma, uçtan uca test, API çıktı örneği
...
Bölüm 4'teki taslak şema gerçek dosyayla karşılaştırılıp netleştirilecek,
...
Notebook 1'den itibaren kodlamaya başlanacak.
